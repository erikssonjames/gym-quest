...
export default function Body () {
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="grid gap-3 sm:grid-cols-3"><MetricCard tone="info" label="Workouts" value={workoutsPending ? "..." : String(workouts?.length ?? 0)} detail={isMine ? "Plans in your library" : "Public plans"} /><MetricCard tone="success" label="Sessions" value={sessionsPending ? "..." : String(sessions?.length ?? 0)} detail={isMine ? "Completed sessions" : "Visible activity"} /><MetricCard tone="warning" label="Badges" value={badgesPending ? "..." : String(unlockedBadges)} detail={isMine ? "Milestones unlocked" : "Selected badge"} /></div>

      {isMine ? <>
        <PageSection title="Training snapshot" description="A quick view of the work behind your profile."><div className="grid gap-3 md:grid-cols-3"><Card><CardContent className="flex items-start gap-3 p-5"><Dumbbell className="mt-0.5 size-4 text-info" /><div><p className="font-medium">Workout library</p><p className="mt-1 text-sm text-muted-foreground">{workouts?.length ?? 0} plan{workouts?.length === 1 ? "" : "s"} ready to use.</p></div></CardContent></Card><Card><CardContent className="flex items-start gap-3 p-5"><CalendarDays className="mt-0.5 size-4 text-success" /><div><p className="font-medium">Recent rhythm</p><p className="mt-1 text-sm text-muted-foreground">{sessions?.[0]?.endedAt ? "Your latest session is recorded." : "Complete a session to start your record."}</p></div></CardContent></Card><Card><CardContent className="flex items-start gap-3 p-5"><Award className="mt-0.5 size-4 text-warning" /><div><p className="font-medium">Achievement progress</p><p className="mt-1 text-sm text-muted-foreground">{unlockedBadges} milestone{unlockedBadges === 1 ? "" : "s"} unlocked.</p></div></CardContent></Card></div></PageSection>
